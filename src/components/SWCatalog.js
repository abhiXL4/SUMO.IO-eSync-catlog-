...
//   useEffect(() => {
//     const fetchData = async () => {
//       const ecuItems = await getEcuItems();
//       setData(ecuItems);
//     };
//     fetchData();
//   }, []); // Reload table on data update
...
//     return acc;
//   }, {});
//  // Log grouped data structure here
...
//           </thead>
//           <tbody>
//   {Object.keys(groupedData).map((ecuName, index) => (
//     <React.Fragment key={`${ecuName}_${index}`}>
//       {/* First row with summary only */}
//       <tr className="border-t" key={`${ecuName}_${groupedData[ecuName][0].catalogueId}`}>
//         <td className="p-2" onClick={() => toggleExpand(ecuName)}>
//           <div className="flex items-center cursor-pointer">
//             {expandedECUs[ecuName] ? <FiChevronDown /> : <FiChevronRight />}
//             <span className="ml-2 font-semibold">{ecuName}</span>
...
//             <FiCheckCircle title="Check List" />
//           </button>
//           <button className="text-gray-600 hover:text-gray-800 p-1" onClick={() => handleDownload(groupedData[ecuName][0].catalogueId, 'sw')}>
//             <FiDownload title="Download" />
//           </button>
...
//       {/* Expanded rows for detailed data */}
//       {expandedECUs[ecuName] && groupedData[ecuName].map((item, itemIndex) => (
//         <tr key={`${item.catalogueId}_${itemIndex}`} className="border-t bg-gray-200">
//           <td className="p-2 pl-8">{item.ecuName}</td>
//           <td className="p-2 text-center">{item.swType || item.configType || 'N/A'}</td>
...
//           <td className="p-2 text-center flex justify-center space-x-1">
//             <button className="text-gray-600 hover:text-gray-800 p-1" onClick={() => handleView(ecuName, item.catalogueId)}>
//               <FiEye title="View" />
//             </button>
//             <button className="text-gray-600 hover:text-gray-800 p-1">
//               <FiCheckCircle title="Check List" />
  ...
  return (
    <div className="bg-white shadow rounded-lg w-full h-screen flex flex-col">
      ...
      <div className="p-4 flex justify-between items-center border-b bg-gray-100">
        ...
        <button className="bg-green-500 text-white px-4 py-2 rounded" onClick={handleUpload}>
          {catalogFilter === 'CONFIG CATALOG' ? 'Upload Config' : 'Upload SW'}
        </button>
      </div>
            ...
            {Object.keys(groupedData).map((ecuName, index) => (
              <React.Fragment key={`${ecuName}_${index}`}>
                ...
                {expandedECUs[ecuName] && groupedData[ecuName].map((item, itemIndex) => (
                  <tr key={`${item.catalogueId}_${itemIndex}`} className="border-t bg-gray-200">
                    <td className="p-2 pl-8">{item.ecuName}</td>
                    <td className="p-2 text-center">{item.swType}</td>
                    <td className="p-2 text-center">{item.releaseVersion}</td>
                    <td className="p-2 text-center">{item.catalogueId}</td>
                    <td className="p-2 text-center">{item.updatedBy}</td>
                    <td className="p-2 text-center">{item.uploadedDate}</td>
                    <td className="p-2 text-center flex justify-center space-x-1">
                      <button className="text-gray-600 hover:text-gray-800 p-1" onClick={() => handleView(item.ecuName, item.catalogueId)}>
                        <FiEye title="View" />
                      </button>
                      <button className="text-gray-600 hover:text-gray-800 p-1">
                        <FiCheckCircle title="Check List" />
                      </button>
                      <button className="text-gray-600 hover:text-gray-800 p-1" onClick={() => handleDownload(item.catalogueId, 'sw')}>
                        <FiDownload title="Download" />
                      </button>
                    </td>
                  </tr>
